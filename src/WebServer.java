...
public class WebServer {
	...
	static class TestHandler implements HttpHandler
	{
    	...
    	{
    		System.out.println("\n->[" + Clock.systemDefaultZone().instant().toString() + "] Request " + t.getProtocol().toString() + "/" + t.getRequestMethod() + " from " + t.getRemoteAddress().toString());
    		// load HTML from resources
   			FileReader file = new FileReader("src\\test-client.html");
   			String response = (new BufferedReader(file)).lines().collect(Collectors.joining());
   			file.close();
			// send response			
    ...
    static class ExecHandler implements HttpHandler
    {
    	...
    	{
    		config = pdf = null;
			configName = pdfName = null;
			byte[] buf = new byte[800 * 1024]; // TODO: IMPLEMENT buffer refilling !
			byte[] bound = ("--" + boundary + "\r\n").getBytes();
			byte[] nl2 = "\r\n\r\n".getBytes();

			int b0 = 0, b1 = 0;
			while (b1 < buf.length) {
				final int len = body.read(buf, b0, buf.length - b0);
				if (len < 1)
					break;
				b1 = b0 + len;
				b0 = b1;
			...
			body.close();
			int i0 = indexOf(buf, bound, 0);
			while (i0 >= 0) {
				int h = indexOf(buf, nl2, i0);
				int i = indexOf(buf, bound, h);
				i0 += bound.length;
				String header = new String(buf, i0, h - i0);
				String ctype = getVal(header, "Content-Type: ");
				String cdisp = getVal(header, "Content-Disposition: ");
				h += nl2.length;
				final int size = (( i < 0 ) ? b1 : i ) - h; 
//					System.out.println("==================================");
//					System.out.println("@Header (" + i0 + "-" + h + "):\n" + header);
//					System.out.println("@Data (" + h + "-" + (h + size) + "), " + (size / 1024) + " kB:\n" + new String(buf, h, size));
				i0 = i;
				onFormField(cdisp, ctype, Arrays.copyOfRange(buf, h, h + size));
			}
    	...
    	public void handle(HttpExchange t) throws IOException
    	{
			...
			try { 
	    		...
	    		if ((null == ctype) || !ctype.startsWith(mpart)) {
	    			...
	    			Main.execute(command, path + configName, path + pdfName, yaml ? null : (path + outFileName));
